# Information:
# This script generates stack barplot about the percent of nodes being activated 
# in region of interest. It takes the absolute EF value. 

library('ProjectTemplate')
load.project()
abs<-roi_abs %>% select(-c("min","max","avg")) %>% mutate(val=act) %>%
...
# Figure 
froi_a<-ggplot(data=NULL,aes(x=region,y=val,fill=session))+
  ...
  labs(x="Region of interest",y="Activation % (absolute EF)")