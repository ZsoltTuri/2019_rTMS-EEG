...
# normal component of the EF in region of interest. 
...
neg<-roi_normal %>% select(-c("min","max","mean_glob","mean_pos","act_pos")) %>% mutate(val=mean_neg) %>%
  mutate(session=plyr::mapvalues(.$session,as.character(c("LI","MI","HI")),
                                 as.character(c("LI1","MI1","HI1"))),
         hemisphere=plyr::mapvalues(.$hemisphere,as.character(c("L","R")),
...
pos<-roi_normal %>% select(-c("min","max","mean_glob","mean_neg","act_neg")) %>% mutate(val=mean_pos) %>%
...
# Figure 
froi2<-ggplot(data=NULL,aes(x=region,y=val,fill=session))+
  ...
  geom_hline(yintercept=-1,linetype="solid",color="blue",size=0.2,alpha=1)+
  theme_bw()+
  ...
  labs(x="Region of interest",y="Normal EF [mV/mm]")+
  coord_cartesian(ylim=c(fparam[['axmin']],fparam[['axmax']]))

ggsave("graphs/roi_EF/roi2.png",plot=froi2,width=240,height=120,units="mm")
rm(neg,pos)
