...
# This script creates figures about phase locking values (plv) locked to the rTMS cessation.
...
fplv1<-ggplot(data=plv)+
  ...
  geom_segment(aes(x=-2.00,y=fparam[["ymin"]],xend=-2.0,yend=fparam[["ymax"]]),linetype='dashed')+
  ...
  stat_summary(mapping=aes(x=time,y=plv_par,colour=protocol),
               fun.y=mean,
               geom="line",
               size=1.5)+
  stat_summary(mapping=aes(x=time,y=plv_par,fill=protocol,colour=NA),
  ...
  guides(color=FALSE)
ggsave("graphs/plv/plv1.png",plot=fplv1,width=210,height=100,units="mm",dpi=300)
