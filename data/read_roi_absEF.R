...
# This script reads the absolute EF values from each region of interest (ROI) into a R dataframe.
...
for(fname in input[["ls"]]){
  ...
  roi_abs<-rbind(roi_abs,temp)}
rm(temp,fname)

# Prepare dataframe
roi_abs$pid<-rep(c(input[["pid_gyr"]],input[["pid_sul"]]),times=input[["ses_len"]])
roi_abs$hemisphere<-rep(c(input[["hem_gyr"]],input[["hem_sul"]]),times=input[["ses_len"]])
roi_abs$region<-rep(c(input[["reg_gyr"]],input[["reg_sul"]]),times=input[["ses_len"]])
roi_abs$session<-rep(input[["ses"]],each=input[["ses_len_long"]])

# Change factor orders
roi_abs$session<-factor(roi_abs$session,levels=c("LI","MI","HI"))
roi_abs$hemisphere<-factor(roi_abs$hemisphere,levels=c("L","R"))
roi_abs$region<-factor(roi_abs$region,levels=c("G_postcentral","G_parietal_sup","G_precuneus","G_pariet_inf-Supramar",
                                               "G_pariet_inf-Angular","G_occipital_sup","G_occipital_middle","G_cuneus",
                                               "G_oc-temp_med-Lingual","G_oc-temp_lat-fusifor","G_and_S_occipital_inf","Pole_occipital",
